# Error Normalization Specification (RFC 9457)

## Purpose

Define the contract for global FastAPI exception handlers that produce RFC 9457 `application/problem+json` responses for all error conditions in the webmarket API. Domain exceptions (`ServiceError` hierarchy), auth `HTTPException` (401/403), and Pydantic `RequestValidationError` MUST all flow through these handlers, returning a unified error schema with `type`, `title`, `status`, `detail`, and `instance` fields. Admin (`/admin/*`) and age-verification endpoints with their own `try/except` handlers MUST NOT be normalized.

This capability supersedes the per-router `try/except ServiceError → HTTPException` translation contract previously defined in the `service-layer` spec. See `openspec/specs/service-layer/spec.md` for the historical service-layer design.
